...
export default function HomePage() {
  const [code, setCode] = useState(
    () => localStorage.getItem(DRAFT_KEY) ?? STARTER_CODE,
  );
  ...
  useEffect(() => {
    ...
    if (resourceCode !== null) {
      setCode(resourceCode);
      localStorage.setItem(DRAFT_KEY, resourceCode);
      localStorage.removeItem('cforge-resource-code');
    }
  }, []);
  ...
  const [outputState, setOutputState] =
    useState<OutputState>('ready');

  const [mobilePanel, setMobilePanel] =
    useState<'editor' | 'output'>('editor');

  const [settingsOpen, setSettingsOpen] = useState(false);
  const [input, setInput] = useState('');
  const [stdout, setStdout] = useState('');
  const [stderr, setStderr] = useState('');
  const [exitCode, setExitCode] =
  ...
  const applyCompilerMarkers = useCallback(
    (message: string) => {
      const monaco = monacoRef.current;
      const editor = editorRef.current;
      ...
      // GCC diagnostics commonly look like:
      // main.c:4:5: error: message

      for (const line of message.split(/\r?\n/)) {
        const match = line.match(
          /(?:main\.c|[^:]+):(\d+):(\d+)(?::\s*(?:fatal )?error:\s*(.*))?/,
        );
    ...
    async (
      ...
      try {
        ...
        try {
          ...
          throw new Error(
            'Compiler server returned an invalid response.',
          );
        }
        ...
        const hasCompilerDiagnostic =
          /(?:main\.c|[^:]+):\d+:\d+:\s*(?:fatal )?error:/m.test(
            result.stderr || '',
          );
        ...
        setStderr(
          'Compiler server is unavailable. Please try again.',
        );
      } finally {
        if (runPhaseTimer) {
          window.clearTimeout(
    ...
    const checkHealth = () => {
      ...
      return fetch(
        ...
        .then(async (response) => {
          ...
          setHealth(
            response.ok &&
              payload?.compilerAvailable !== false
              ? 'online'
              : 'offline',
          );
        })
  ...
  return (
    <section className="compiler-page">
      ...
      <div className="compiler-workspace">
        ...
        >
          ...
          <div className="terminal">
            <div
              className={`terminal-state ${outputState}`}
            >
    ...
    <label className="setting-row">
      <span>
        ...
        <small>
          {label ===
          'Auto-save'
            ? 'Restore the local draft after refresh.'
            : 'Apply this editor preference.'}
        </small>
      </span>
